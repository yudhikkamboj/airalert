...
```bash
AirAlert$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
AirAlert$ sam local start-api
AirAlert$ curl http://localhost:3000/
```
...
```bash
AirAlert$ sam logs -n HelloWorldFunction --stack-name AirAlert --tail
```
...
```bash
AirAlert$ cd HelloWorldFunction
...
```bash
sam delete --stack-name AirAlert
```
